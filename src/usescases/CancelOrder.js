...
class CancelOrder {
    ...
    execute() {
        this.setOrderState();
        this.setOrderDateTimeUpdated();
        var updatedOrder = this.cancelOrder();
        this.sendEmailCancelationOrder();
        return updatedOrder;
    }

    cancelOrder() {
        return this.orderRepository.setState(this.order);
    }

    setOrderState() {
        this.order.state = "inactive";
    }

    setOrderDateTimeUpdated() {
        this.order.datetimeupdated = moment().format('YYYY-MM-DD HH:mm:ss');
    }

    sendEmailCancelationOrder() {
    ...
    createEmailBodyFromOrder() {
        ...
        this.order.products.forEach(element => {
            ...
            htmlProductTemp = htmlProductTemp.replace('@datetime', this.order.datetimeupdated);

            htmlProducts = htmlProducts.concat(htmlProductTemp);
        });

        htmlBody = htmlBody.replace('@code', this.order.code);
        htmlBody = htmlBody.replace('@datetime', this.order.datetimeupdated);
